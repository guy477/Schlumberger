...
inp = np.loadtxt('data/LIQUIDRATEIC01.txt')
print(inp)
...
for i in range(len(xchart)):
    ...
    for g in range(288):
        if not(i*288 + g+1 > len(inp)):
            new_row.append((np.log(inp[i*288 + g]) - np.log(962.6729705
))/np.log(962.6729705))
        else:
...
layout = go.Layout(
    title='Liquid Rate of IC01',
    xaxis = dict(ticks='', nticks=36),
    yaxis = dict(ticks='' )
)

fig = go.Figure(data=data, layout=layout)
py.plot(fig, filename='liquid-rate-IC01')

...
inp = np.loadtxt('data/ICO2_CHOKE_POSITION.TXT')
print(inp)
...
for i in range(len(xchart)):
    ...
    for g in range(288):
        if not(i*288 + g+1 > len(inp)):
            new_row.append((np.log(inp[i*288 + g]) - np.log(81.2934151))/np.log(81.2934151))
        else:
...
py.plot(fig, filename='choke_position-heatmap-IC02')
